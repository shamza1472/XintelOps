...
from xintelops.vector.deduplicator_hash import HashDeduplicator
...
class PipelineRunner:
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self.hash_dedup = HashDeduplicator(self.settings, self.db)
        self.vector_dedup = Deduplicator(self.settings, self.db) if self.settings.uses_external_llm else None
        self.email = ResendClient(self.settings)
        self.verifier = VerifierAgent() if self.settings.uses_external_llm else None
        self.analyst = AnalystAgent() if self.settings.uses_external_llm else None
        self.red_team = RedTeamAgent() if self.settings.uses_external_llm else None
        self.strategist = StrategistAgent() if self.settings.uses_external_llm else None

    def ingest_and_store(self) -> tuple[list, str]:
        items = self.ingest.fetch_all()
        bundle = self.ingest.bundle_text(items)
    ...
    def run(self, skip_ingest: bool = False, source_bundle: str | None = None) -> dict[str, Any]:
        if self.settings.uses_cursor_llm:
            raise RuntimeError(
                "Cursor-native mode: run scripts/run_ingest.py, analyze with Cursor, "
                "then scripts/finalize_scan.py. See docs/CURSOR_AUTOMATION.md"
            )

        if source_bundle is None:
            if skip_ingest:
                raise ValueError("source_bundle is required when skip_ingest=True")
            _, source_bundle = self.ingest_and_store()
    ...
    def _store_ingested_items(self, items) -> None:
        ...
        novel_count = 0
        for item in items:
            if self.settings.uses_external_llm and self.vector_dedup:
                dedup_result, embedding = self.vector_dedup.process(item)
                ...
                dedup_result = self.hash_dedup.process(item)
                self.db.insert_raw_feed(
                    source=item.source,
                    raw_text=item.raw_text,
                    title=item.title,
